<div class="space-y-6">
    ...
    <x-electrik::page-header
        title="Subscription"
        description="Manage the current team subscription."
    />
        ...
        @if ($subscription && $plan)
            <dl class="grid gap-4 sm:grid-cols-2">
                ...
                @if ($subscription->ends_at)
                    <div>
                        <dt class="text-sm text-muted-foreground">Ends</dt>
            ...
            <div class="mt-6 flex flex-wrap gap-2 border-t border-border pt-4">
                @if ($subscription->onGracePeriod())
                    <x-slate::button type="button" wire:click="resume">Resume</x-slate::button>
                ...
                @endif
                <x-slate::button as="a" href="{{ route('billing.plans') }}" variant="ghost" wire:navigate>Change plan</x-slate::button>
            </div>
        @else
